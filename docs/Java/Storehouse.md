...
Стартовый экран приложения показан на рисунке 1.1. Ему соответствует класс `LoginActivity`.
...
Листинг кода метода onCreate с инициализацией `ScorocodeSdk`.
...
На данном экране вводятся все необходимые поля документа (характеризующие пользователя). Регистрация нового пользователя происходит при помощи метода `.register` класса `User`. Пример использования данного метода:
...
При успешной регистрации пользователя будет выполнен метод `onRegisterSucceed(...) callback` интерфейса иначе будет выполнен метод `onRegisterFailed(...)`.
...
* Создаем новый документ коллекции. В нашем случае имя коллекции «storehouse» хранится в статической переменной `COLLECTION_NAME` класса `MainActivity`.
* Присваиваем его полям необходимые значения.
* Сохраняем документ.

В случае если мы создаем новый документ без ассоциации с документом в БД, т.е. без использования метода `.getDocumentById(…)`, то при сохранении программа вставляет новый документ в БД. Данные документы хранятся в БД Scorocode в виде показанном на рисунке 2.3
...
Поиск по платформе задается при помощи метода `.equalTo(field, value)` класса `Query`.

Поиск по доступным цветам задается при помощи метода `.containedIn` класса `Query`.

Таким образом мы формируем запрос (query) для указанной коллекции БД и получаем экземпляры класса `DocumentInfo` характеризующие документы удовлетворяющие данному запросу. Эту информацию мы передаем в активность при помощи callback и обновляем там информацию.
